...
st.write("أدخل بيانات الحقل والمناخ في القائمة الجانبية، وسيقوم الذكاء الاصطناعي بحساب الإنتاج المتوقع.")

# ==========================================
# 2. Load Model (using Booster — works on all xgboost versions)
# ==========================================
@st.cache_resource
def load_model():
    booster = xgb.Booster()
...
# The exact 30 features the model was trained on (order matters)
FEATURE_NAMES = [
...
# ==========================================
st.sidebar.header("📝 إدخال بيانات الحقل")

st.sidebar.subheader("1. بيانات الحقل")
altitude   = st.sidebar.number_input("الارتفاع عن سطح البحر (متر)", 500, 1500, 800)
...
if st.button("🚀 توقع الإنتاج الآن", use_container_width=True):
    with st.spinner("جاري تحليل البيانات..."):
        ...
        pruning_dict = {"Light": 1, "Moderate": 2, "Heavy": 0}  # matches LabelEncoder order
        ...
        prediction = max(0, prediction)  # cannot be negative
        ...
        if prediction >= field_mean * 1.1:
            st.success(f"🌟 موسم ممتاز! الإنتاج أعلى من المتوسط بنسبة {((prediction/field_mean)-1)*100:.1f}%")
        elif prediction >= field_mean * 0.9:
            st.info("✅ موسم جيد. الإنتاج قريب من المتوسط التاريخي للحقل.")
        else:
            st.warning(f"⚠️ موسم أقل من المتوسط بنسبة {(1-(prediction/field_mean))*100:.1f}%")

        st.markdown("---")
        st.subheader("📋 ملخص بيانات الحقل")
        summary = pd.DataFrame({
            "العامل": ["الارتفاع", "الأمطار السنوية", "حرارة الصيف", "أيام الصقيع",
        ...
        st.table(summary)
        st.success("تم حساب النتيجة بناءً على 30 عاملاً زراعياً ومناخياً باستخدام نموذج XGBoost.")
...
with st.expander("ℹ️ معلومات عن النموذج"):
    ...
    **نموذج XGBoost لتنبؤ إنتاج زيت الزيتون — منطقة الشوف، لبنان**
    ...
    |---------|--------|
    | عدد الميزات | 30 ميزة |
    | حجم بيانات التدريب | 200 حقل-موسم |
